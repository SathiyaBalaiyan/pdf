...
//multicell method
$pdf->Cell(35,5,"MultiCell method",0,0);

$item = "This is a quite long text for a cell. Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. This is normal length for a cell. This is also a quite long text for a cell. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat.";
...
{	
	...
	if($pdf->GetStringWidth($item) < $cellWidth)
	{
	...
	else
	{
		...
		$textLength=strlen($item);	//total text length
		...
		{   //loop until end of text
			...
			{
				$maxChar++;
				$tmpString=substr($item,$startChar,$maxChar);
			}
	...
	// //write the cells
	// $pdf->Cell(10,($line * $cellHeight),$item[0],0,0); //adapt height to number of lines
	// $pdf->Cell(60,($line * $cellHeight),$item[1],0,0); //adapt height to number of lines
	...
	$xPos=$pdf->GetX();
	$yPos=$pdf->GetY();
	$pdf->MultiCell($cellWidth,$cellHeight,$item,0);
	
	//return the position for next cell next to the multicell
	//and offset the x with multicell width
	$pdf->SetXY($xPos + $cellWidth, $yPos);
	
	// $pdf->Cell(40,($line * $cellHeight),$item[3],0,1); //adapt height to number of lines	
}
...
// 	//check whether the text is overflowing
// 	if($this->GetStringWidth($item) < $cellWidth)
// 	{
// 		//if not, then do nothing
// 		$line=1;
// 	}
...
// 	// //write the cells
// 	// $this->Cell(10,($line * $cellHeight),$item[0],0,0); //adapt height to number of lines
// 	// $this->Cell(60,($line * $cellHeight),$item[1],0,0); //adapt height to number of lines
...
// 	$xPos=$this->GetX();
// 	$yPos=$this->GetY();
// 	$this->MultiCell($cellWidth,$cellHeight,$item,0);
	
// 	//return the position for next cell next to the multicell
// 	//and offset the x with multicell width
// 	$this->SetXY($xPos + $cellWidth, $yPos);
	
// 	// $this->Cell(40,($line * $cellHeight),$item[3],0,1); //adapt height to number of lines	
// }
?>
